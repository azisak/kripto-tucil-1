...
class Application(Frame):
    def __init__(self, master=None):
        ...
        inputFrame = Frame()
        Label(inputFrame, text="Original Text",
        ...
        Button(selectionFrame, text='Encrypt', command=self.encrypt).grid(
            row=2, column=0, padx=5, pady=4)
        Button(selectionFrame, text='Decrypt', command=self.decrypt).grid(
        ...
        Label(outputFrame, text="Processed Text",
              font=("Helvetica", 14)).grid(row=0)
        Button(outputFrame, text='Save', command=self.savefile).grid(row=1)
        outputFrame.grid(row=3)
        self.eOutput.grid(row=3, column=1)

    def savefile(self):
        if (type(self.cipherOption.getCipher()) is cp.VigenereExtended):
            messagebox.showwarning(
                "Warning", "Tidak bisa digunakan fitur ini untuk Vigenere Extended")
        elif (len(self.eOutput.get("1.0", "end-1c")) == 0):
            messagebox.showwarning(
                "Warning", "Tolong encrypt sesuatu!")
        else:
            filename =  filedialog.asksaveasfilename(title = "Select file",filetypes = (("jpeg files","*.txt"),("all files","*.*")))
            f = open(filename,"w")
        ...
        if (type(self.cipherOption.getCipher()) is cp.VigenereExtended):
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", "Make sure you browse text file for non Extended vigenere cipher")
    ...
    def validate(self):
        if (len(self.eInput.get("1.0", "end-1c")) == 0):
            ...
            messagebox.showwarning(
                "Warning", "File masih kosong atau Hanya bisa enkripsi/dekripsi file to file untuk Vigenere Extended")
            return False
        else:
            return True
